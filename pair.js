...
const config = {
    ...
    OTP_EXPIRY: 300000,
    OWNER_NUMBERS: ['94789226579', '94789117233'],
...
async function handleDownloadCommand(socket, message, command, args, settings, language) {
    ...
    switch (command) {
        case 'song':
            // YouTube song download implementation
            await sendFormattedMessage(socket, jid, '🎵 Song Download', 
                'Searching for song...', language);
            // Implement ytdl or similar
            break;
...
`;
            ...
            const buttons = [
                { buttonId: `${config.PREFIX}video 144 ${query}`, buttonText: { displayText: '144p' }, type: 1 },
                { buttonId: `${config.PREFIX}video 240 ${query}`, buttonText: { displayText: '240p' }, type: 1 },
                { buttonId: `${config.PREFIX}video 360 ${query}`, buttonText: { displayText: '360p' }, type: 1 },
                { buttonId: `${config.PREFIX}video 720 ${query}`, buttonText: { displayText: '720p' }, type: 1 },
                { buttonId: `${config.PREFIX}video 1080 ${query}`, buttonText: { displayText: '1080p' }, type: 1 }
            ];
        ...
        case 'tiktok':
            ...
            await sendFormattedMessage(socket, jid, '📱 TikTok Download', 
                'Processing TikTok URL...', language);
        ...
        case 'fb':
            ...
            await sendFormattedMessage(socket, jid, '📘 Facebook Download', 
                'Processing Facebook URL...', language);
...
📞 OWNER NUMBERS:
• Lakshan: +94789226579
• Savendra: +94789117233

🏢 BOT DETAILS:
• 🤖 Name: LAKSHAN-MD (Laki Mini Bot)
• 🎯 Version: v1.0.0
